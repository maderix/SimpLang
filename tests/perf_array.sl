fn kernel_main() {
    ...
    // Test 3: Random access pattern (cache-unfriendly)
    var random_sum = 0.0;
    iter = 0;
    while (iter < iterations) {
        i = 0;
        while (i < size) {
            ...
            var random_idx = (i * 7919) % size;  // 7919 is a prime
    ...
    while (iter < iterations) {
        i = 0;
        while (i < size) {
            var stride_idx = (i * stride) % size;
            stride_sum = stride_sum + large_array[stride_idx];
            i = i + 1;
        }
        iter = iter + 1;
    }
