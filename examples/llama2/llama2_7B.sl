// LLaMA 2 7B Model
// Config: dim=4096, n_layers=32, n_heads=32, vocab=32000, seq_len=512
// ~7B parameters
...
fn llama2_forward(
    ...
    f32[] hb,       // hidden dimension buffer [hidden_dim=11008]
    ...
    // Hyperparameters
    var head_size = dim / n_heads;  // 128
    var kv_dim = (dim * n_kv_heads) / n_heads;  // 4096
    ...
    while (layer < n_layers) {
        ...
        q = matmul(wq, xb, q, 4096i, 4096i, 1i, qkv_offset, 0i, 0i);

        // K = wk @ xb
        k = matmul(wk, xb, k, 4096i, 4096i, 1i, qkv_offset, 0i, 0i);

        // V = wv @ xb
        v = matmul(wv, xb, v, 4096i, 4096i, 1i, qkv_offset, 0i, 0i);
        ...
        while (h < n_heads) {
            ...
            while (t < att_len) {
                ...
                score = score / 11.3;  // sqrt(head_size=96) ≈ 9.75
        ...
        xb2 = matmul(wo, xb, xb2, 4096i, 4096i, 1i, wo_offset, 0i, 0i);
        ...
        hb = matmul(w1, xb, hb, 11008i, 4096i, 1i, ffn_offset, 0i, 0i);
        ...
        hb = matmul(w3, xb, hb, 11008i, 4096i, 1i, ffn_offset, 0i, 0i);
        ...
        xb = matmul(w2, hb, xb, 4096i, 11008i, 1i, w2_offset, 0i, 0i);
    ...
    logits = matmul(wcls, xb, logits, 32000i, 4096i, 1i, 0i, 0i, 0i);
