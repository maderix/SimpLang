// LLaMA 2-7B Fully Quantized Model - NO FP32
// W8A8 matmuls with INT32 accumulation (VNNI)
// INT16/INT32 for all element-wise operations
//
// Architecture:
//   dim = 4096, n_layers = 32, n_heads = 32, n_kv_heads = 32 (no GQA)
//   head_dim = 128, hidden_dim = 11008, vocab_size = 32000
...
// ============================================================================
// Decode: Single token forward pass for 7B (NO GQA - n_kv_heads = n_heads)
// ============================================================================
fn llama2_7b_decode_i8(
    i8[] x_i8,                // [dim=4096]
    i16[] rms_att_w,          // [n_layers=32, dim=4096]
    i16[] rms_ffn_w,          // [n_layers=32, dim=4096]
    i16[] rms_final_w,        // [dim=4096]
    i8[] wq_t,                // [n_layers * dim * dim]
    i8[] wk_t,                // [n_layers * dim * dim] (no GQA)
    i8[] wv_t,                // [n_layers * dim * dim] (no GQA)
    i8[] wo_t,                // [n_layers * dim * dim]
    i8[] w1_t,                // [n_layers * hidden_dim * dim]
    i8[] w2_t,                // [n_layers * dim * hidden_dim]
    i8[] w3_t,                // [n_layers * hidden_dim * dim]
    i8[] wcls_t,              // [vocab_size=32000, dim=4096]
    i16[] xb_i16,             // [dim=4096]
    i8[] xb_i8,               // [dim=4096]
    i32[] q_i32,              // [dim=4096]
    i32[] k_i32,              // [dim=4096]
    i32[] v_i32,              // [dim=4096]
    i32[] attn_out,           // [dim=4096]
    i8[] ffn_hb,              // [hidden_dim=11008]
    i8[] k_cache,             // [n_layers, max_seq_len, dim]
    i8[] v_cache,             // [n_layers, max_seq_len, dim]
    i32[] att_scores,         // [n_heads=32, max_seq_len]
    i8[] att_probs,           // [n_heads=32, max_seq_len]
    i16[] cos_tab,            // [max_seq_len, head_dim/2=64]
    i16[] sin_tab,            // [max_seq_len, head_dim/2=64]
    i32[] logits,             // [vocab_size=32000]
    i64 pos,
    i64 dim,                  // 4096
    i64 hidden_dim,           // 11008
    i64 n_layers,             // 32
    i64 n_heads,              // 32
    i64 n_kv_heads,           // 32 (no GQA)
    i64 vocab_size,           // 32000
    i64 max_seq_len,          // 2048
    i64 head_dim              // 128
) -> i32 {
    var kv_dim = n_kv_heads * head_dim;  // 4096 (same as dim for no GQA)
    var n_rep = n_heads / n_kv_heads;     // 1
    var seq_len = pos + 1i;
    ...
    while (layer < n_layers) {
        ...
        // Attention RMSNorm
        var dummy = rmsnorm_i16_7b(xb_i16, rms_att_w, xb_i16, dim, layer_offset);
        ...
        // QKV Projections
        i8<1, 4096> xb_mat = tensor_from_array(xb_i8, 0i);

        var wq_offset = layer * dim * dim;
        i8<4096, 4096> wq_mat = tensor_from_array(wq_t, wq_offset);
        var Q = tensor_matmul(xb_mat, wq_mat);

        var wk_offset = layer * dim * dim;
        i8<4096, 4096> wk_mat = tensor_from_array(wk_t, wk_offset);
        var K = tensor_matmul(xb_mat, wk_mat);

        var wv_offset = layer * dim * dim;
        i8<4096, 4096> wv_mat = tensor_from_array(wv_t, wv_offset);
        ...
        dummy = rope_i32_7b(q_i32, cos_tab, sin_tab, q_i32, dim, head_dim, rope_offset);
        dummy = rope_i32_7b(k_i32, cos_tab, sin_tab, k_i32, dim, head_dim, rope_offset);

        // Store K, V in cache
        var cache_offset = layer * max_seq_len * dim + pos * dim;
        i = 0i;
        while (i < dim) {
        ...
        // Multi-Head Attention (NO GQA - each head has own KV)
        i = 0i;
        while (i < dim) {
            attn_out[i] = 0i;
            i = i + 1i;
        ...
        while (h < n_heads) {
            ...
            while (t < seq_len) {
                var score = 0i;
                var k_pos = layer * max_seq_len * dim + t * dim + head_offset;
                var d = 0i;
                while (d < head_dim) {
            ...
            dummy = softmax_i32_to_i8_7b(att_scores, att_probs, seq_len, att_offset, att_offset);
            ...
            while (d < head_dim) {
                ...
                t = 0i;
                while (t < seq_len) {
                    var v_pos = layer * max_seq_len * dim + t * dim + head_offset;
        ...
        i8<1, 4096> attn_mat = tensor_from_array(xb_i8, 0i);
        var wo_offset = layer * dim * dim;
        i8<4096, 4096> wo_mat = tensor_from_array(wo_t, wo_offset);
        ...
        // FFN RMSNorm
        dummy = rmsnorm_i16_7b(xb_i16, rms_ffn_w, xb_i16, dim, layer_offset);
        ...
        i8<1, 4096> ffn_in = tensor_from_array(xb_i8, 0i);
        i8<4096, 11008> w1_mat = tensor_from_array(w1_t, w1_offset);
        var Gate = tensor_matmul(ffn_in, w1_mat);

        i8<4096, 11008> w3_mat = tensor_from_array(w3_t, w3_offset);
        ...
        i8<1, 11008> hb_mat = tensor_from_array(ffn_hb, 0i);
        i8<11008, 4096> w2_mat = tensor_from_array(w2_t, w2_offset);
    ...
    // Final RMSNorm
    var dummy2 = rmsnorm_i16_7b(xb_i16, rms_final_w, xb_i16, dim, 0i);
    ...
    // Classifier
    i8<1, 4096> final_mat = tensor_from_array(xb_i8, 0i);
    i8<4096, 32000> wcls_mat = tensor_from_array(wcls_t, 0i);
    var Logits = tensor_matmul(final_mat, wcls_mat);
...
// ============================================================================
// Prefill 32 tokens for 7B
// ============================================================================
fn llama2_7b_prefill_32_i8(
    i8[] x_i8,
    ...
    var kv_dim = n_kv_heads * head_dim;  // 4096 (no GQA)
    var n_rep = n_heads / n_kv_heads;     // 1
    ...
    while (layer < n_layers) {
        ...
        i8<32, 4096> xb_mat = tensor_from_array(xb_i8, 0i);
        ...
        // Store KV cache
        var cache_layer_offset = layer * seq_len * dim;
        t = 0i;
        while (t < seq_len) {
            var i = 0i;
            while (i < dim) {
                ...
                if (v_val > 127i) { v_val = 127i; }
                if (v_val < -128i) { v_val = -128i; }
                k_cache[cache_layer_offset + t * dim + i] = k_val as i8;
                v_cache[cache_layer_offset + t * dim + i] = v_val as i8;
                i = i + 1i;
            }
            t = t + 1i;
        }

        // Causal Attention (no GQA)
        var h = 0i;
        while (h < n_heads) {
            var head_offset = h * head_dim;
            ...
            while (q_pos < seq_len) {
                ...
                while (k_pos <= q_pos) {
                    ...
                    var d = 0i;
                    while (d < head_dim) {
                        score = score + Q[q_pos, head_offset + d] * (k_cache[cache_layer_offset + k_pos * dim + head_offset + d] as i32);
                        d = d + 1i;
                    }
                ...
                while (d < head_dim) {
                    ...
                    var vi = 0i;
                    while (vi <= q_pos) {
                        val = val + (att_probs[att_offset + vi] as i32) * (v_cache[cache_layer_offset + vi * dim + head_offset + d] as i32);
                        vi = vi + 1i;
                    }
        ...
        i8<32, 4096> attn_mat = tensor_from_array(xb_i8, 0i);
        var wo_offset = layer * dim * dim;
        i8<4096, 4096> wo_mat = tensor_from_array(wo_t, wo_offset);
        var Wo_out = tensor_matmul(attn_mat, wo_mat);
        ...
        i8<32, 4096> ffn_in = tensor_from_array(xb_i8, 0i);
        i8<4096, 11008> w1_mat = tensor_from_array(w1_t, w1_offset);
        var Gate = tensor_matmul(ffn_in, w1_mat);
        ...
        i8<32, 11008> hb_mat = tensor_from_array(ffn_hb, 0i);
        i8<11008, 4096> w2_mat = tensor_from_array(w2_t, w2_offset);
        var Down = tensor_matmul(hb_mat, w2_mat);
