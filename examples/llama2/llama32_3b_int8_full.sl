// LLaMA 3.2-3B Fully Quantized Model - NO FP32
...
//   head_dim = 128, hidden_dim = 8192, vocab_size = 128256

// ============================================================================
// INT16 RMSNorm (same as 1B)
// ============================================================================
fn rmsnorm_i16_3b(
...
// ============================================================================
// INT32 Softmax -> INT8 (same as 1B)
// ============================================================================
fn softmax_i32_to_i8_3b(
...
// ============================================================================
// INT32 RoPE (adjusted for head_dim=128)
// ============================================================================
fn rope_i32_3b(
...
// ============================================================================
// Decode: Single token forward pass for 3B
// ============================================================================
fn llama32_3b_decode_i8(
    i8[] x_i8,                // [dim=3072]
    i16[] rms_att_w,          // [n_layers=28, dim=3072]
    i16[] rms_ffn_w,          // [n_layers=28, dim=3072]
    i16[] rms_final_w,        // [dim=3072]
    i8[] wq_t,                // [n_layers * dim * dim]
    i8[] wk_t,                // [n_layers * kv_dim * dim] kv_dim=1024
    ...
    i8[] wcls_t,              // [vocab_size=128256, dim=3072]
    i16[] xb_i16,             // [dim=3072]
    i8[] xb_i8,               // [dim=3072]
    i32[] q_i32,              // [dim=3072]
    i32[] k_i32,              // [kv_dim=1024]
    i32[] v_i32,              // [kv_dim=1024]
    i32[] attn_out,           // [dim=3072]
    i8[] ffn_hb,              // [hidden_dim=8192]
    i8[] k_cache,             // [n_layers, max_seq_len, kv_dim]
    i8[] v_cache,             // [n_layers, max_seq_len, kv_dim]
    i32[] att_scores,         // [n_heads=24, max_seq_len]
    i8[] att_probs,           // [n_heads=24, max_seq_len]
    i16[] cos_tab,            // [max_seq_len, head_dim/2=64]
    i16[] sin_tab,            // [max_seq_len, head_dim/2=64]
    i32[] logits,             // [vocab_size=128256]
    i64 pos,
    i64 dim,                  // 3072
    i64 hidden_dim,           // 8192
    i64 n_layers,             // 28
    ...
    var kv_dim = n_kv_heads * head_dim;  // 1024
    ...
    while (layer < n_layers) {
        ...
        // Attention RMSNorm
        var dummy = rmsnorm_i16_3b(xb_i16, rms_att_w, xb_i16, dim, layer_offset);
        ...
        // QKV Projections
        i8<1, 3072> xb_mat = tensor_from_array(xb_i8, 0i);

        var wq_offset = layer * dim * dim;
        i8<3072, 3072> wq_mat = tensor_from_array(wq_t, wq_offset);
        var Q = tensor_matmul_nt(xb_mat, wq_mat);

        var wk_offset = layer * kv_dim * dim;
        i8<1024, 3072> wk_mat = tensor_from_array(wk_t, wk_offset);
        var K = tensor_matmul_nt(xb_mat, wk_mat);

        var wv_offset = layer * kv_dim * dim;
        i8<1024, 3072> wv_mat = tensor_from_array(wv_t, wv_offset);
        ...
        dummy = rope_i32_3b(q_i32, cos_tab, sin_tab, q_i32, dim, head_dim, rope_offset);
        dummy = rope_i32_3b(k_i32, cos_tab, sin_tab, k_i32, kv_dim, head_dim, rope_offset);
        ...
        while (h < n_heads) {
            ...
            while (t < seq_len) {
                ...
                att_scores[h * max_seq_len + t] = score / 11i;  // 1/sqrt(128) ~ 1/11
                t = t + 1i;
            }

            var att_offset = h * max_seq_len;
            dummy = softmax_i32_to_i8_3b(att_scores, att_probs, seq_len, att_offset, att_offset);
        ...
        i8<1, 3072> attn_mat = tensor_from_array(xb_i8, 0i);
        var wo_offset = layer * dim * dim;
        i8<3072, 3072> wo_mat = tensor_from_array(wo_t, wo_offset);
        ...
        // FFN RMSNorm
        dummy = rmsnorm_i16_3b(xb_i16, rms_ffn_w, xb_i16, dim, layer_offset);
        ...
        i8<1, 3072> ffn_in = tensor_from_array(xb_i8, 0i);
        i8<8192, 3072> w1_mat = tensor_from_array(w1_t, w1_offset);
        var Gate = tensor_matmul_nt(ffn_in, w1_mat);

        i8<8192, 3072> w3_mat = tensor_from_array(w3_t, w3_offset);
        ...
        i8<3072, 8192> w2_mat = tensor_from_array(w2_t, w2_offset);
    ...
    // Final RMSNorm
    var dummy2 = rmsnorm_i16_3b(xb_i16, rms_final_w, xb_i16, dim, 0i);
    ...
    // Classifier
    i8<1, 3072> final_mat = tensor_from_array(xb_i8, 0i);
    i8<128256, 3072> wcls_mat = tensor_from_array(wcls_t, 0i);
...
// ============================================================================
// Prefill 32 tokens for 3B
// ============================================================================
fn llama32_3b_prefill_32_i8(
    i8[] x_i8,
    ...
    while (layer < n_layers) {
        ...
        i8<32, 3072> xb_mat = tensor_from_array(xb_i8, 0i);
        ...
        while (h < n_heads) {
            ...
            while (q_pos < seq_len) {
                ...
                while (k_pos <= q_pos) {
                    ...
                    var d = 0i;
                    while (d < head_dim) {
                        score = score + Q[q_pos, q_head_offset + d] * (k_cache[cache_layer_offset + k_pos * kv_dim + kv_head_offset + d] as i32);
                        d = d + 1i;
                    }
        ...
        i8<32, 3072> attn_mat = tensor_from_array(xb_i8, 0i);
        var wo_offset = layer * dim * dim;
        i8<3072, 3072> wo_mat = tensor_from_array(wo_t, wo_offset);
        var Wo_out = tensor_matmul_nt(attn_mat, wo_mat);
        ...
        i8<32, 3072> ffn_in = tensor_from_array(xb_i8, 0i);
        i8<8192, 3072> w1_mat = tensor_from_array(w1_t, w1_offset);
        var Gate = tensor_matmul_nt(ffn_in, w1_mat);
